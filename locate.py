...
def mistaken():
    ...
    try:
        print('程序出现异常，可能由于卡顿')
        print('如果多次输出错误，可能是分辨率异常。请选择1440*810')
        print('程序重新启动')
    except:
        mistaken()
...
def give_spells(i,troops):
    photo='./troops/'+str(i)+'.png'
    Part=pyautogui.locateOnScreen(photo, confidence=0.96, grayscale=False)
    while Part!=None:       # <class 'pyscreeze.Box'> 
        Center=pyautogui.center(Part)
        rgb=pyautogui.screenshot().getpixel((int(Center.x+random.randint(-10,10)),int(Center.y+random.randint(-10,10))))
        if rgb[0]==rgb[1] and rgb[1]==rgb[2]:
...
def main():
        ...
        Part_help=pyautogui.locateAllOnScreen('./others/help.png', confidence=0.9, grayscale=False)
        ...
        for all_help in Part_help:
            ...
            # for i in (13,6,19,9,14,17,16,22,10,12,7,23,3,2,1,'c','e','h','j','G3','G2','G1'):
            for i in (13,6,10,22,'c','e',17,16,14,3,1,2,'h'):
                photo='./troops/'+str(i)+'.png'
                Part = pyautogui.locateOnScreen(photo, confidence=0.9, grayscale=False)         # <class 'pyscreeze.Box'>
                if Part==None:
                    print('未识别到图片'+str(i))
                    continue
                if type(i)==int:
